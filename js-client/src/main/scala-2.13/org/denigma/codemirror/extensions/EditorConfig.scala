...
import org.denigma.codemirror.{Editor, EditorConfiguration}
import scala.scalajs.js

/**
 * Builders for easier configuration
 */
object EditorConfig extends EditorConfigurationBuilder(OptMap.noOpts) {
  override def result: EditorConfiguration = super.result
  
  override def value(code: String): EditorConfigurationBuilder = {
    super.value(code)
  }

  override def mode(modeValue: String): EditorConfigurationBuilder = {
    super.mode(modeValue)
  }

  override def theme(themeValue: String): EditorConfigurationBuilder = {
    super.theme(themeValue)
  }

  override def lineNumbers(value: Boolean): EditorConfigurationBuilder = {
    super.lineNumbers(value)
  }

  override def gutters(value: js.Array[String]): EditorConfigurationBuilder = {
    super.gutters(value)
  }
}
...
class EditorConfigurationBuilder(val dict: OptMap)
  ...
  def onDragEvent(dragEventHandler: js.Function2[Editor, js.Any, Boolean]) = jsOpt("onDragEvent", dragEventHandler)

  def onKeyEvent(keyEventHandler: js.Function2[Editor, js.Any, Boolean]) = jsOpt("onKeyEvent", keyEventHandler)
